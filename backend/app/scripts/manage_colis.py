...
def create_colis_entry(
    db: Session, fedex_id: str, description: str = "Colis créé via script"
):
...
def main():
    ...
    if not args:
        ...
        print(
            "Commands: create [fedex_id] [description], "
            "bulk_create [fedex_id1] [fedex_id2] ..., get [identifier], "
            "list, update [id] [status] [location], delete [id]"
        )
        return
    ...
    if command == "bulk_create":
        ...
        db = get_db_session()
        colis_service = ColisService(db)
        colis_list, total = colis_service.search_colis(
            ColisFilter(page=1, page_size=1000)
        )  # List up to 1000 for simplicity
