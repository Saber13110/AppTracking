...
class FedExService:
    def __init__(
        self, account: str | None = None, config_path: str | None = None
    ):
        try:
    ...
    def _authenticate(self) -> None:
        ...
        # Update both instance and global cache inside the lock for thread
        # safety
        with _token_lock:
    ...
    async def get_proof_of_delivery(self, tracking_number: str) -> bytes:
        ...
        try:
            ...
            url = (
                f"{self.base_url}/track/v1/shipments/"
                f"{tracking_number}/proof-of-delivery"
            )
            headers = {
    ...
    def _create_location(self, location_data: Dict[str, Any]) -> Location:
        ...
        address = location_data.get("locationContactAndAddress", {}).get(
            "address", {}
        )
        return Location(
            ...
            coordinates=(
                {
                    "latitude": location_data.get("coordinates", {}).get(
                        "latitude"
                    ),
                    "longitude": location_data.get("coordinates", {}).get(
                        "longitude"
                    ),
                }
    ...
    async def track_package(self, tracking_number: str) -> TrackingResponse:
        ...
        try:
            ...
            if not tracking_data.get("output", {}).get(
            ...
            track_result = tracking_data["output"]["completeTrackResults"][0][
                "trackResults"
            ][0]
    ...
    def _format_tracking_info(
        self, tracking_details: Dict[str, Any]
    ) -> TrackingInfo:
        try:
            ...
            origin = self._extract_location(
                tracking_details.get("shipperInformation", {}).get(
                    "address", {}
                )
            )
            destination = self._extract_location(
                tracking_details.get("recipientInformation", {}).get(
                    "address", {}
                )
            )
            ...
            return TrackingInfo(
                ...
                tracking_url=(
                    f"https://www.fedex.com/tracking?tracknumbers="
                    f"{tracking_number}"
                ),
            )
        except Exception as e:
    ...
    def _extract_location(self, location_data: Dict[str, Any]) -> Location:
        try:
            ...
            if (latitude is None or longitude is None) and location_data.get(
                "coordinates"
            ):
    ...
    def _extract_package_details(
        ...
        try:
            ...
            return PackageDetails(
                ...
                service_type=tracking_details.get("serviceDetail", {}).get(
                    "type", ""
                ),
                signature_required=False,
                # Will be updated based on delivery details
                special_handling=[],
                declared_value=0.0,
                customs_value=0.0,
    ...
    def _extract_delivery_details(
        ...
        try:
            ...
            for option in delivery_details.get(
                "deliveryOptionEligibilityDetails", []
            ):
                delivery_option_eligibility[option.get("option", "")] = (
                    option.get("eligibility", "") == "ELIGIBLE"
                )
    ...
    def _extract_commercial_info(
        ...
        try:
            ...
            for identifier in additional_info.get("packageIdentifiers", []):
                for value in identifier.get("values", []):
                    package_identifiers.append(
                        {"type": identifier.get("type", ""), "value": value}
                    )
            ...
            return CommercialInfo(
                ...
                service_detail=tracking_details.get("serviceDetail", {}).get(
                    "description", ""
                ),
