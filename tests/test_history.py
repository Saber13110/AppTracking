...
def test_post_history_endpoint(db_session):
    ...
    payload = TrackedShipmentCreate(tracking_number="ABC", status="OK", pinned=True)
...
def test_patch_history_endpoint(db_session):
    user = create_user(db_session)
    payload = TrackedShipmentCreate(tracking_number="DEF")
    record = asyncio.run(history_router.add_history(payload, user, db_session))
    update = TrackedShipmentUpdate(note="u", pinned=True)
    updated = asyncio.run(history_router.update_history(record.id, update, user, db_session))
    assert updated.note == "u"
